...
const TopNav = ({isNotActive , setNotActive}) => {
  var barsIcon =  <i className="fas fa-bars fs-2"></i>
  ...
  return (
    <Navbar  bg="white" expand="lg" style={{ zIndex:"444444",position : 'fixed' , top : '0' ,width : '100%'}}>
      <Container fluid>
      <button
          type="button"
          ...
          className="btn"
        >
          <span  className={ isNotActive ? 'text-primary' : 'hidden' }>{ barsIcon }</span>
          <span  className={ isNotActive ? 'text-primary hidden' : 'text-danger ' }>{ crossIcon }</span>
        </button>
        <Navbar.Brand
